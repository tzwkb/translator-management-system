...
先启动服务：.venv/bin/python app.py
再另开终端：.venv/bin/python test_acceptance.py
...
def main():
    ...
    print("=== C PRD 硬规则 ===")
    req("POST", "/api/translators", {"name": "唯A", "email": "dup@x.com"}, token=ET)
    chk("create重复邮箱400", code(lambda: req("POST", "/api/translators", {"name": "唯B", "email": "dup@x.com"}, token=ET)) == 400)
    chk("update改成已存在邮箱400", code(lambda: req("PUT", "/api/translators/1", {"name": "张明", "email": "dup@x.com"}, token=ET)) == 400)
