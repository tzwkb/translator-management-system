...
for (const id of ["p-settlement_month", "q-period", "capacityMonth", "capacity-detail-month"]) {
  assert.match(html, new RegExp(`id="${id}"[^>]+type="month"`), `${id} should use month input`);
}

for (const id of ["f-punctuality_rate", "q-score"]) {
  assert.match(html, new RegExp(`id="${id}"[^>]+min="0"[^>]+max="100"`), `${id} should be limited to 0-100`);
}

for (const id of ["p-word_count", "p-rate", "a-orig", "a-new", "x-ded", "pe-volume"]) {
